...
## **Лекция, 24 ноября 2022** Обслуживание рекурсии

1. Вызов / возврат

    Добавляем в модель вычислений операцию PUSH и POP

    ``Картинка порядка действий программного стека для y <- sin(x)``

    ``Картинки стекового кадра на каждое из действий``

    f_{call / Ret}(1) = 2 (кол-во параметров + 1 от адреса возврата + 1 от регистров + 1)
...
2. Факториал
    ```
        ...
        End
    ```
    ДИСКЛЕЙМЕР: ДАЛЬШЕ ОБМАН

    Делаем копию функци, чтобы было как с тем, когда тангенс вызывает синус,  и так каждый раз, когда нам что-то нужно. БОЛЬШЕ КОПИЙ!

    ``Рисунок обмана, цепочка вызовов и возвратов``

    ПРАВДА

    В реальности это дело скомплируется в реентерабельный модуль, программный стек хранит всю историю, но на деле копий нет.

3. Порождённое дерево рекурсии

    Порождённое чем? Алгоритмом и конкретным входом D

    ``Рисунок дерева - унарного для F(4)``

    Дерево может быть не только унарное (например, в случае Фибоначчи)

    Для анализа рекурсивного алгоритма нужно знать информацию о дереве:
    - R(D) - общее число вершин
    - R_v(D) - внутренние вершины (порождаемые)
    - R_L(D) - листья (остановка рекурсии)
    - H(D) - высота дерева

4. Анализ трудоёмкости
    У нас уже есть:
    - f_{call/return}(1) = 2(k + 3)
    - f_L(1)

    f_A(D) = f_{call/return}(1) \cdot R(D) \cdot f_L(1) \cdot R_L(D) + \sum_i \sum_j f_{V_{ij}}(D)

    F_A(k) = 8 \cdot (k + 1) + 2 \cdot 1 + 4 \cdot k = 12k + 10
